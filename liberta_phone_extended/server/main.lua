...
AddEventHandler('esx_phone:startCall', function(number)
  ...
  if foundPlayer ~= false then
    ...
    TriggerClientEvent('esx_phone:endCall', _source, 'Le contact n'est pas disponnible')
  end
    ...
    function(result)
      ...
      if foundNumber then
        ...
        if phoneNumber == xPlayer.get('phoneNumber') then
          TriggerClientEvent('esx:showNotification', _source, 'Vous ne pouvez pas ajouter votre propre numero')
        else
          ...
          if hasAlreadyAdded then
            TriggerClientEvent('esx:showNotification', _source, 'Vous avez deja ce numéro dans vos contacts')
          else
            ...
            MySQL.Async.execute(
              ...
              function(rowsChanged)
                ...
                TriggerClientEvent('esx:showNotification', _source, 'Contacte ajouter')
        ...
        TriggerClientEvent('esx:showNotification', source, 'Ce numero n existe pas')
      end
...
AddEventHandler('esx_phone:billCall', function(duration)
  ...
  TriggerClientEvent('esx:showNotification', _source, 'Votre facture telephonique est de ~r~$' .. amount .. '~s~')
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
  ...
  if amount > 0 and amount <= xPlayer.getAccount('bank').money then
    ...
    TriggerClientEvent('esx:showNotification', _source, 'Vous transferer ~g~$' .. amount .. '~s~ à ' .. targetXPlayer.name)
