-- Optimisation et et arrangement du script par Utox#1387. --
		...
		MySQL.Async.fetchAll(
			...
			function(result)
				...
				MySQL.Async.execute(
					...
					function (rowsChanged)
					TriggerClientEvent('esx:showNotification', source, "Vous avez acheté une nouvelle carte SIM")
					end
				)
		end)
end

ESX.RegisterServerCallback('liberta:CheckSim', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('liberta:sim_give')
AddEventHandler('liberta:sim_give', function (store, index)
  local _source = source
  local id = index
  local xPlayer = ESX.GetPlayerFromId(_source)
  ...
  TriggerClientEvent('esx:showNotification', id, "Quelqu'un vous a remis une carte SIM,le numéro est : ~o~" .. simZ)
...
RegisterServerEvent('liberta:sim_delete')
AddEventHandler('liberta:sim_delete', function (sim)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local sim = sim
...
RegisterServerEvent('liberta:sim_use')
AddEventHandler('liberta:sim_use', function (sim)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	--local new = sim
...
ESX.RegisterServerCallback('liberta:GetList', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local cartesim = {}
...
ESX.RegisterServerCallback('liberta:GetNameZ', function(source, cb, id)
